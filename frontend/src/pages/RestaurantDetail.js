...
import {clearSelectedRestaurant, fetchRestaurantById} from '../redux/slices/restaurantSlice';
import {
  clearReviews,
  createReview,
  deleteReview,
  ...
  return (
    <Container maxWidth="lg" sx={{my: 4}}>
        ...
        <Grid container>
          <Grid item xs={12} md={7} lg={8}>
            <Box sx={{borderBottom: 1, borderColor: 'divider'}}>
              <Tabs value={currentTab} onChange={handleTabChange} aria-label="restaurant details tabs"
                    variant="scrollable" scrollButtons="auto">
                <Tab label="Overview" id="restaurant-tab-0" aria-controls="restaurant-tabpanel-0"/>
                <Tab label={`Reviews (${restaurant.reviewCount || 0})`} id="restaurant-tab-1"
            ...
            <TabPanel value={currentTab} index={0}>
              ...
              <Grid container spacing={1}>
                <Grid item xs={12} sm={6} sx={{display: 'flex', alignItems: 'center', mb: 1}}>
                  ...
                  <Typography
                    variant="body2">{`${restaurant.addressStreet}, ${restaurant.addressCity}, ${restaurant.addressState} ${restaurant.addressZip}`}</Typography>
                </Grid>
                <Grid item xs={12} sm={6} sx={{display: 'flex', alignItems: 'center', mb: 1}}>
            ...
            <TabPanel value={currentTab} index={1}>
              ...
              {isAuthenticated && currentUserRole === 'CUSTOMER' && !editingReview && (
                <Box component="form" onSubmit={handleNewReviewSubmit}
                  ...
                  <Button type="submit" variant="contained" endIcon={<SendIcon/>}
                          disabled={mutationStatus === 'pending' || !newReviewRating}>
              ...
              {/* --- Edit Review Form --- */}
              {editingReview && (
                <Box component="form" onSubmit={handleUpdateReviewSubmit}
                     sx={{mb: 4, p: 2, border: '1px dashed blue', borderRadius: 1}}>
                  <Typography variant="h6" gutterBottom>Edit Your Review</Typography>
                  <Rating
                    name="edit-review-rating"
                    value={editingReview.rating}
                    onChange={(event, newValue) => {
                      setEditingReview(prev => ({...prev, rating: newValue}));
                    }}
                    sx={{mb: 1}}
                  />
                  <TextField
                    ...
                    <Paper key={review.id} elevation={2} sx={{p: 2, mb: 2}}>
                      <Box sx={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 0.5}}>
                        <Typography variant="subtitle1"
                                    sx={{fontWeight: 'bold'}}>{review.customerName || 'Anonymous'}</Typography>
                        ...
                        {isAuthenticated && review.customerId === currentUserId && (
                          <Box>
                            <IconButton size="small" onClick={() => handleEditReview(review)}
                                        disabled={mutationStatus === 'pending' || !!editingReview}>
          ...
          <Grid item xs={12} md={5} lg={4}
                sx={{borderLeft: {md: '1px solid'}, borderColor: {md: 'divider'}, p: {xs: 2, md: 3}}}>
            <Paper variant="outlined" sx={{p: 2, backgroundColor: 'grey.50'}}>
              ...
              <Box
                sx={{height: 200, width: '100%', mb: 1, borderRadius: 1, overflow: 'hidden', border: '1px solid #ddd'}}>
                <iframe
      ...
      <Dialog open={showConfirmDeleteDialog} onClose={() => setShowConfirmDeleteDialog(false)}>
        ...
        <DialogContent><DialogContentText>Are you sure you want to delete this
          review?</DialogContentText></DialogContent>
        <DialogActions>
