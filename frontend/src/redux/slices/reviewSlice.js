...
const reviewSlice = createSlice({
  ...
  reducers: {
    ...
    clearReviews: (state) => {
      state.reviews = [];
      state.pagination = { currentPage: 0, totalPages: 0, totalElements: 0, size: 10 };
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchReviewsForRestaurant.fulfilled, (state, action) => {
        ...
        state.pagination = {
          currentPage: action.payload.data.number !== undefined ? action.payload.data.number : 0,
          totalPages: action.payload.data.totalPages !== undefined ? action.payload.data.totalPages : 0,
          totalElements: action.payload.data.totalElements !== undefined ? action.payload.data.totalElements : 0,
          size: action.payload.data.size !== undefined ? action.payload.data.size : 10,
        };
      })
